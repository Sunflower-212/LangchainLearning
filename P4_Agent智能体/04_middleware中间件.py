...
def log_before_agent(state:AgentState,runtime:Runtime)->None:
    # angent 执行前会调用这个函数并传入state和runtime两个对象
...
agent = create_agent(
    ...
    tools=[get_weather],
    middleware=[log_before_agent,log_after_agent,log_after_model,log_before_model,model_call_hook,tool_call_hook],
    system_prompt="你是一个天气查询助手，能够调用工具查询指定城市的天气。"
)

res = agent.invoke(
    {
        "messages":[{"role":"user","content":"武汉今天天气如何？"}]
    }
)
print(res)
